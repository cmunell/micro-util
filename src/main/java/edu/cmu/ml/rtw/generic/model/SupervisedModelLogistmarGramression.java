...
/**
 * 
 * SupervisedModelLogistmarGramression implements the 'feature grammar'
 * model described in 
...
public class SupervisedModelLogistmarGramression<D extends Datum<L>, L> extends SupervisedModel<D, L> {
	...
	protected class Likelihood extends AbstractStochasticFunctionUsingDataSet<LabeledDataInstance<Vector, Double>> {
		...
		public Likelihood(Random random, DataFeatureMatrix<D, L> arkDataSet) {
			...
			this.dataSet = 
					...
					PlataniosUtil.makePlataniosDataSet((DataFeatureMatrix<Datum<Boolean>, Boolean>)arkDataSet, SupervisedModelLogistmarGramression.this.weightedLabels, 1.0/5.0, true, true);
		...
		private void extendDataSet(Vector c_p, Vector c_n) {
			...
			for (Integer featureToExpand : featuresToExpand) {
				...
				for (Entry<String, Obj> entry : featureChildObjs.entrySet()) {
					...
					FeatureTokenSpanFnFilteredVocab<D, L> featureChild = (FeatureTokenSpanFnFilteredVocab<D, L>)this.arkDataSet.getData().getDatumTools().makeFeatureInstance(featureChildFunction.getName(), SupervisedModelLogistmarGramression.this.context);
					...
					.debugWriteln("(" + this.arkDataSet.getReferenceName() + ") Feature " + featureToExpand + " (" + featureObj.getReferenceName() + "-" + featureVocabStr + ") c=(" + c_p.get(featureToExpand) + "," + c_n.get(featureToExpand) + ") expanded with rule " + entry.getKey() + " (" + featureChild.getVocabularySize() + ")...");
				}
			}
		}
	...
	public SupervisedModelLogistmarGramression(DatumContext<D, L> context) {
	...
	public boolean train(DataFeatureMatrix<D, L> data, DataFeatureMatrix<D, L> testData, List<SupervisedModelEvaluation<D, L>> evaluations) {
		...
		output.debugWriteln("Logistmar gramression (" + data.getReferenceName() + ") training for at most " + maximumIterations + 
		...
		this.u = new AdaptiveGradientSolver.Builder(new Likelihood(data.getData().getDatumTools().getDataTools().makeLocalRandom(), data), this.u)
						.lowerBound(0)
		...
		output.debugWriteln("Logistmar gramression (" + data.getReferenceName() + ") finished training."); 
	...
	protected AssignmentList toParseInternalHelper(
		...
		internalAssignments.add(Assignment.assignmentTyped(null, Context.ObjectType.ARRAY.toString(), "bias", bias));
		...
		for (Entry<Integer, Pair<Double, Double>> cEntry : cList) {
			...
			internalAssignments.add(Assignment.assignmentTyped(null, Context.ObjectType.ARRAY.toString(), "u_" + cEntry.getKey() + ((constructedFeature) ? "_c" : ""), weight));
		}
		...
		for (Entry<Integer, List<Integer>> entry : this.childToParentFeatureMap.entrySet()) {
			...
			internalAssignments.add(Assignment.assignmentTyped(null, Context.ObjectType.ARRAY.toString(), "cToP_" + entry.getKey(), array));
		}
